...
class ExoSOFTpriors(object):
    ...
    def ecc_prior_fn(self, ecc):
        ...
        ret = 1.0
        if ecc!=0:
            if (self.e_prior is True) or (self.e_prior is '2e'):
    ...
    def p_prior_fn(self, p):
        ret = 1.0
        if p!=0.0:
            if (self.p_prior is True) or (self.p_prior is 'log'):
    ...
    def inc_prior_fn(self, inc):
        ...
        if inc not in [0.0,90.0,180.0]:
            mn = self.inc_min*(const.pi/180.0)
            mx = self.inc_max*(const.pi/180.0)
            inc_rad = inc*(const.pi/180.0)
            if (self.inc_prior is True) or (self.inc_prior is 'sin'):
                ret = np.sin(inc_rad) / np.abs(np.cos(mn)-np.cos(mx))
                
            elif self.inc_prior is 'cos':
    ...
    def m2_prior_fn(self, m2, m1):
        ...
        if 0.0 not in [m2,m1]:
            # First check if a gauss prob should be also calculated.
            if self.m2_est!=self.m2_err!=0:
                ret*=self.gaussian(m2, self.m2_est, self.m2_err)
            # Then caculate requested mass function prior.
            if (self.m2_prior is True) or (self.m2_prior is "CMF"):
                ret*=self.cmf_prior(m2, m1)
                
            elif self.m2_prior is "PDMF":
                ret*=self.pdmf_prior(m2)
                
            elif self.m2_prior is "IMF":
    ...
    def para_prior_fn(self, para):
        ...
        if para!=0.0:
            ...
            if self.para_est!=self.para_err!=0:
                ...
                ret*=self.gaussian(para, self.para_err, self.para_est)
                #print('para gauss ',self.gaussian(para, self.para_err, self.para_est))
        return ret
